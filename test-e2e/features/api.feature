...
#
  Scenario: Verify simple send with headers as file
    When I send 'GET' request to "https://jsonplaceholder.typicode.com/todos/1" with headers "$textFile('testData/headers.json')" and save response as 'response'
    Then Response "$response" Status Code to be equal '200'
    And Response "$response" Status Message to be equal 'OK'
    Then Response "$response.payload" contains:
      | userId    |
...
#
  ...
  Scenario: Verify simple send with query and headers as file
    When I send 'GET' request to "https://jsonplaceholder.typicode.com/posts" with headers "$textFile('testData/headers.json')" with qs "?userId=1" and save response as 'response'
    Then Response "$response" Status Code to be equal '200'
    And Response "$response" Status Message to be equal 'OK'
  ...
  Scenario: Verify POST with valid request body as Cucumber Doc String and headers as file
    When I send "POST" request and save response as "response" to "https://jsonplaceholder.typicode.com/posts" with headers "$textFile('testData/headers.json')" with Body:
      """
      {
        "userId": 1,
        "id": 1,
  ...
  Scenario: Verify POST with valid request body as file and headers as file
    When I send "POST" request to "https://jsonplaceholder.typicode.com/posts" with headers "$textFile('testData/headers.json')" with Body "$textFile('testData/test_data_file.json')" and save response as "response"
    Then Response "$response" Status Code to be equal '201'
    And Response "$response" Status Message to be equal 'Created'
    And Response "$response.payload" contains:
      | userId    |
